...
import tde4
from  export_nuke_LD_3DE4_Lens_Distortion_Node  import *

#prj name env setings
###########################################################################################################
if os.environ['SG_PRJNAME']:
    prjName = os.environ['SG_PRJNAME']
    project_name = str(prjName)
else:
    self.logger.error("ENV VARIABLE -> SG_PRJNAME not set.")
###########################################################################################################
...
class Tde4SessionPublishPlugin(HookBaseClass):
    ...
    def settings(self):
        ...
        base_settings = super(Tde4SessionPublishPlugin, self).settings or {}
    ...
    def accept(self, settings, item):
        ...
        # print("##########################################################")
        # print("PUBLISH SESSION.PY -> ACCEPT -> path %s-> "%(path))
        # print("##########################################################")
        ...
        if not path:
            ...
            self.logger.warn(
                "The 3de4 session has not been saved.",
                extra=_get_save_as_action()
            )

        self.logger.info(
            "3de4 '%s' plugin accepted the current 3de4 session." %
            (self.name,)
        )
        return {
            "accepted": True,
    ...
    def validate(self, settings, item):
        ...
        return super(Tde4SessionPublishPlugin, self).validate(settings, item)
    ...
    def publish(self, settings, item):
        ...
        super(Tde4SessionPublishPlugin, self).publish(settings, item)
        _export_mel(path)
        _export_Nuke_LensDitortion(path)
    ...
    def finalize(self, settings, item):
        ...
        super(Tde4SessionPublishPlugin, self).finalize(settings, item)

        # bump the session file to the next version
        self._save_to_next_version(item.properties["path"], item, _save_session)
        #copy the mel script to the correct folder
        mel_pathExp = "/mnt/assets/"+project_name+"/sequences/Jay_SEQ001/JY_SH001/MMV/work/maya_export"
        _copyToMelPath(path,mel_pathExp)
        #copy the nuke script to the correct folder
        nuke_pathExp = "/mnt/assets/"+project_name+"/sequences/Jay_SEQ001/JY_SH001/MMV/work/nuke_export"
        _copyToNukeDistPath(path,nuke_pathExp)
...
def _export_Nuke_LensDitortion(nuke_path):
    """
    Exports Lens Distortion file from 3de4 to nuke 
    """
# camera and lens
...
# Get smart start value for initial frame option menu.
    ...
    initial_frame_nuke = initial_frame_3de4
    if not nuke_path:
...
# export
    try:
        ...
        raise Exception('Cannot run Lens distotion Nuke Export, function error.')


def processFileClean(CorrectExport_path):
    extractNm = "."+CorrectExport_path.rsplit('.',2)[1]
    cleanFileNm = CorrectExport_path.replace(extractNm,"")
    shutil.move(os.path.abspath(CorrectExport_path),os.path.abspath(cleanFileNm))

def copyToExport(Export_path,CFilePath):
    if os.path.exists(os.path.abspath(Export_path)):
        mv = shutil.move(os.path.abspath(CFilePath), os.path.abspath(Export_path))
        processFileClean(mv)
...
def _copyToNukeDistPath(s_path,nukeExpPath):
    ensure_folder_exists(os.path.abspath(nukeExpPath))
    nukeFilePath = s_path +".nk" 
    if os.path.isfile(os.path.abspath(nukeFilePath)):
        copyToExport(nukeExpPath,nukeFilePath)
