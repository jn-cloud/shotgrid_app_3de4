...
class Tde4ExportNukePublishPlugin(HookBaseClass):
    """
    ...
    def name(self):
        ...
        return "Tde4ExportNukePublishPlugin"
    ...
    def description(self):
        ...
        return """
        Publishes the Nuke Lens Distortion file to Shotgun. A <b>Publish</b> entry will be
    ...
    def settings(self):
        ...
        base_settings = super(Tde4ExportNukePublishPlugin, self).settings or {}
    ...
    def validate(self, settings, item):
        ...
        return super(Tde4ExportNukePublishPlugin, self).validate(settings, item)
    ...
    def publish(self, settings, item):
        ...
        try:
            # excecute nuke lens distortion
            _export_Nuke_LensDitortion(path)
        except Exception:
            raise Exception("Unable to export Nuke lens distotion script")
    ...
    def finalize(self, settings, item):
        ...
        publish_fl_name = publish_name + "_v" + str(publish_version).zfill(3) + ".nk"

        #get path from a specified template
        Npublish_path = self.get_SpecificTemplatePublishPath(path,publish_version,"3de4_shot_LensDistNuke_export")

        publishedNukePath = os.path.join(os.path.abspath(Npublish_path),publish_fl_name)

        #copy published nuke script to the correct folder
        try:
            _copyToNukeDistPath(path,Npublish_path)
        except Exception:
            raise Exception("Unable to export Nuke lens distotion script")
        ...
        try:
            ...
            raise Exception("Unable to publish nuke script")
...
def processFileClean(NukeExport_path):
    extractNm = "."+NukeExport_path.rsplit('.',2)[1]
    cleanFileNm = NukeExport_path.replace(extractNm,"")
    shutil.move(os.path.abspath(NukeExport_path),os.path.abspath(cleanFileNm))

def copyToExportNuke(NukeExport_path,NukeFilePath):
    if os.path.exists(os.path.abspath(NukeExport_path)):
        mv = shutil.move(os.path.abspath(NukeFilePath), os.path.abspath(NukeExport_path))
        processFileClean(mv)
...
def _copyToNukeDistPath(s_path,nukeExpPath):
    ...
    if os.path.isfile(os.path.abspath(nukeFilePath)):
        copyToExportNuke(nukeExpPath,nukeFilePath)
