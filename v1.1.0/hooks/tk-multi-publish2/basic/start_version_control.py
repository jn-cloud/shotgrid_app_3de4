...
class Tde4StartVersionControlPlugin(HookBaseClass):
    """
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "3de4 '%s' plugin rejected the current session..." %
                    (self.name,)
                )
                self.logger.info(
                    "  There is already a version number in the file...")
                self.logger.info("  3de4 file path: %s", path)
        ...
        self.logger.info(
            "3de4 '%s' plugin accepted the current session." %
            (self.name,),
    ...
    def publish(self, settings, item):
        ...
        self.logger.info("A version number has been added to the 3de4 file...")
        self.logger.info("3de4 file path: %s", version_path)
